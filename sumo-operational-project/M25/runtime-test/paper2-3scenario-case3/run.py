...
for scenario in scenarios:
    for seed in seeds:
        ...
        # New variables for second ego vehicle
        ego3_id = 'ego3'
        target_lane_ego3 = '104359041_2'  # Different lane for ego3
        target_position_ego3 = 3000  # Desired position for ego3 to stop
        ego3_stop_time = None
        ego3_stop_flag = False
        ...
        while traci.simulation.getTime() < SIMULATION_END_TIME:
            ...
            if scenario == "attack":
                ...
                if ego2_id in traci.vehicle.getIDList():
                ...
                # Ego vehicle 3 stop logic
                if ego3_id in traci.vehicle.getIDList():
                    ego3_pos = traci.vehicle.getLanePosition(ego3_id)
                    ego3_lane_id = traci.vehicle.getLaneID(ego3_id)

                    if ego3_lane_id[:9] == '104359041':
                        traci.vehicle.changeLane(ego3_id, 2, 100)  # Ego2 to right lane

                    if ego3_lane_id == target_lane_ego3 and not ego3_stop_flag and ego3_pos >= 2900:
                        current_speed_ego3 = traci.vehicle.getSpeed(ego3_id)
                        new_speed_ego3, time_to_stop_ego3 = precise_stop_control(ego3_id, target_position_ego3, current_speed_ego3, ego3_pos)
                        traci.vehicle.setSpeed(ego3_id, new_speed_ego3)

                        if abs(ego3_pos - target_position_ego3) <= 1 and not ego3_stop_flag:
                            traci.vehicle.setSpeed(ego3_id, 0)
                            # print(f"Ego2 has stopped at time {current_time}s.")
                            ego3_stop_flag = True
