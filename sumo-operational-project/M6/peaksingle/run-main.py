...
SUMO_CONFIG = "M6.sumocfg"  # SUMO configuration file
DETECTORS_FILE = "detectors.add.xml"  # XML file defining detectors
SIMULATION_END_TIME = 66600  # Total simulation time (e.g., 360 seconds)
...
INTERVAL_START = 51436.9  # Start time for data collection (shared between scenarios) None if attack scenario is enabled
EGO_BREAKDOWN_DURATION = 86400  # Specify the time in seconds after which the ego vehicle is removed
...
def main():
    ...
    while traci.simulation.getTime() < SIMULATION_END_TIME:
        ...
        if ATTACK_SCENARIO and ego_id in traci.vehicle.getIDList():
            ...
            # print(f"Debug: Ego vehicle position: {ego_pos}, lane: {ego_lane_id}")

            if ego_lane_id[:9] == '204901083':
            ...
            if ego_lane_id == target_lane and not stopFlag and ego_pos >= 2900:
                ...
                if abs(ego_pos - target_position) <= 1 and not stopFlag:
                    ...
                    print(f"Debug: Ego has stop signal sent {ego_lane_id}, position {ego_pos}; timestep: {current_time}")
                    # ego_stop_times[seed] = current_time  # Store the stop time for this seed
                    stopFlag = True

        # # Ego vehicle stop logic
        # if ATTACK_SCENARIO and ego_id in traci.vehicle.getIDList():
        #     if traci.vehicle.getRoadID(ego_id) == 'E1' and traci.vehicle.getLanePosition(ego_id) > 2500:
        #         # Disable safety checks for the 'attack'
        #         traci.vehicle.setSpeedMode(ego_id, 0)
        #         traci.vehicle.setLaneChangeMode(ego_id, 0)

        #         # Calculate time to stop
        #         time_to_stop = traci.vehicle.getSpeed(ego_id) / 9  # Assume deceleration rate of 9 m/s^2
        #         traci.vehicle.slowDown(ego_id, 0, time_to_stop)
        #         traci.vehicle.setSpeed(ego_id, 0)  # Uncomment this line when using step-length of 1s

        # Check for ego vehicle stop
        if ATTACK_SCENARIO and ego_stop_time is None and ego_id in traci.vehicle.getIDList():
            # print(f"Debug: Ego vehicle speed: {traci.vehicle.getSpeed('ego')}")
            if stopFlag == True:
