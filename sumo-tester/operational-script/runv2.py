...
SIMULATION_DURATION = 720  # Total simulation time (e.g., 360 seconds)
TIME_INTERVAL = 60  # Time interval to aggregate vehicle counts
INTERVAL_START = 60  # Start time of the interval
    ...
    while traci.simulation.getTime() < SIMULATION_DURATION:
        ...
        if current_time >= INTERVAL_START: 
            ...
            if current_time >= interval_start_time + TIME_INTERVAL:
                ...
                print(f"Interval {interval_start_time}-{current_time}: {interval_unique_vehicles} vehicles, flow rate = {flowrate} vehicles/hour")
