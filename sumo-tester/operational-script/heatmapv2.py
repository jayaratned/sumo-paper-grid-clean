...
# Debug: Print adjusted time index
print("Adjusted Time Index (Rounded):", [round(x) for x in spr_matrix.index.tolist()])

# Generate interval labels with rounding
interval_labels = [
    f"[{int(round(start))},{int(round(end))})"
    for start, end in zip(
        spr_matrix.index,
        list(spr_matrix.index[1:]) + [spr_matrix.index[-1] + (spr_matrix.index[1] - spr_matrix.index[0])]
    )
]

# Debug: Print generated intervals
print("Generated Intervals (Rounded):", interval_labels)
