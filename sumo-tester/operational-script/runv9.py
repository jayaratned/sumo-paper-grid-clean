        ...
        if cross_section:
            ...
            if key not in combined_results:
                combined_results[key] = {
                    ...
                    "scenario": "attack" if ATTACK_SCENARIO else "normal",
                }
    ...
    while traci.simulation.getTime() < SIMULATION_DURATION:
        ...
        if ATTACK_SCENARIO and ego_stop_time is None and 'ego' in traci.vehicle.getIDList():
            if traci.vehicle.getSpeed('ego') == 0:
                ...
                print(f"Ego vehicle stopped at time: {ego_stop_time}s.")

        # Process each detector
        for detector_id in detector_ids:
            data = detectors_data[detector_id]

            # Start counting only after the ego vehicle stops
            if ego_stop_time is not None and current_time >= ego_stop_time:
                if data["interval_start_time"] == 0:
                    data["interval_start_time"] = ego_stop_time
