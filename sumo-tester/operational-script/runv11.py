...
ATTACK_SCENARIO = True  # Set to True to enable the ego stop logic for attack scenario
INTERVAL_START = None  # Start time for data collection (shared between scenarios)
...
def main():
    ...
    ego_id = 'ego'
    target_lane = 'E1_1'
    target_position = 2501  # Desired position to stop the vehicle on the lane
    ego_stop_time = None
    ...
    while traci.simulation.getTime() < SIMULATION_DURATION:
        ...
        if ATTACK_SCENARIO and ego_id in traci.vehicle.getIDList():
            ...
            if ego_lane_id[:2] == 'E1':
                traci.vehicle.changeLane(ego_id, 1, 100)

            if ego_lane_id == target_lane:
